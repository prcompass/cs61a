...
def accuracy(typed, reference):
    ...
    if total == 0:
        return 0.0
    for i, p in zip(typed_words, reference_words):
        correct += i == p
...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    similiar_word, similiar_diff = min(zip(valid_words, words_diff), key=lambda x: x[1])
    if similiar_diff > limit:
        return user_word
    else:
